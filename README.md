...
<br>
一句话说明白Vue的数据响应式实现原理: 通过proxy换掉对象的get和set方法，在get里获取到哪些函数用了它，set里重新运行那些函数，也就是每次值更新都重新运行依赖它的函数（不过这不代表随便在data里return或者ref弄一个响应式变量，再随便写一个依赖它的函数，在它的值改变的时候都会重新运行，倒也不是这么直接的，不然还要什么methods，computed）
<br>
